...
\title{{Inference Network - A Verified Inference Network} \\
...
\IEEEauthorblockN{December 2025 (v2.0)}%, xyz i\IEEEauthorrefmark{1} and xyzi \IEEEauthorrefmark{1} and xyzz\IEEEauthorrefmark{1} and xyz\IEEEauthorrefmark{1}}
...
This paper introduces Inference Network, a decentralized network facilitating the deployment of AI systems on a Web3 infrastructure. In Inference Network, we utilize cryptographic technologies such as zero-knowledge proofs (ZKP), fully homomorphic encryption (FHE), and multi-party computation (MPC) to safeguard the integrity and privacy of users and/or AI developers. Furthermore, a blockchain-based architecture ensures payment assurances and democratic governance within the system. While some challenges have been addressed, our team remains committed to continuously updating Inference Network with new technologies to tackle emerging challenges in Decentralized AI.
...
Our proposed system, Inference Network, aims to leverage blockchain technology to facilitate the exchange of inferences between model developers and users in a secure and transparent manner. Through the Inference Network, model developers can offer their trained models to users seeking specific predictions or analyses. Users, in turn, can access these inferences by paying with tokens which then creates a decentralized marketplace for AI services. To ensure the integrity and trustworthiness of the exchanged inferences, model developers utilize zero-knowledge proofs (ZKPs) to provide verifiable evidence that the requested computations were indeed performed correctly without revealing any sensitive information about the underlying model or data. By integrating ZKPs into the transaction process, our system enhances transparency and trust, enabling users to confidently engage with model developers while preserving the confidentiality of their data and ensuring the authenticity of the provided inferences. Through this innovative blockchain-based solution, we aim to empower both model developers and users to participate in a secure and efficient marketplace for AI services, fostering a decentralized ecosystem that promotes fairness, transparency, and collaboration.

\section{Technological Foundations: Exploring Key Technologies}
To integrate AI and Web3 and provide a comprehensive list of capabilities, we explore various technologies in this section. Some of these technologies are utilized in designing Inference Network version 1, while others are not yet practical for implementation. However, it's essential to note that Inference Network will continually update to incorporate any advancements in technologies to evolve into a more practical solution over time.

\subsection{Technologies used in Inference Network}

This subsection describes the core technologies utilized in Inference Network.

\subsubsection{Stake-Backed Security}

Inference Network utilizes a stake-backed security model where operators must lock collateral to participate in the network. This cryptoeconomic security mechanism ensures honest behavior: operators who submit incorrect inference results can have their staked tokens slashed, creating a strong financial incentive for correctness.

The security model integrates with existing proof-of-stake infrastructure, allowing the network to inherit economic security from established validator sets. Key components include delegation management for operator registration and stake delegation, allocation management for stake distribution and slashing enforcement, and reward coordination for incentive distribution. A native token facilitates payment for inference services and network governance.
...
Blockchain interconnection, as a critical component for designing Inference Network, refers to the capability of various blockchain networks or platforms to communicate and interact seamlessly. This interoperability facilitates the smooth flow of data, assets, or transactions between disparate blockchain systems, fostering cross-chain functionality and collaboration. There are multiple approaches to achieving blockchain interconnection, including cross-chain communication protocols, atomic swaps, wrapped tokens, sidechains, and oracles. Such interconnection is pivotal for unlocking the complete potential of blockchain technology, enabling collaboration, scalability, and interoperability across diverse blockchain networks and platforms.
...
Inference Network offers a transformative approach for AI operators seeking to transition their off-chain AI models onto blockchain networks while safeguarding their proprietary algorithms. This framework streamlines the intricate process of model conversion, enabling rapid deployment across multiple blockchain ecosystems. It serves as a seamless bridge between the off-chain world of AI and the on-chain realm, ensuring intellectual property remains veiled through the use of zero-knowledge cryptography. By providing a secure and efficient payment infrastructure, Inference Network facilitates atomic value exchange for AI services, paving the way for a new era of autonomous AI agents interacting within the blockchain space.

For consumers of AI predictions, Inference Network offers an additional layer of assurance by eliminating trust assumptions. They confidently rely on the network to validate that the inputs are processed using the correct and intended AI model, with a cryptographic guarantee of faithful execution.  As a result, consumers of AI services benefit from a transparent, trust-minimized environment where AI predictions are verified, reducing the need for blind trust in the operators' execution.

\subsection{Technical Architecture}

To address current blockchain limitations and challenges of running on-chain Neural Networks, Inference Network is designed to serve as a conduit between off-chain and on-chain architectures, as shown in Figure~\ref{fig:Fig 1}. Taking a forward-looking approach, the Inference Network architecture is inherently modular, a design philosophy for each component of the system to be individually updated or replaced.
...
\node[block, right=1.7cm of np] (ua)  {Inference Network SDK\\$1:1$ verifier contract per\\model\\Verifying Keys\\Verified Output Data};
...
\draw[-triangle 45] (z)--(cy);
    ...
    \caption{Inference Network Overview}
    \label{fig:Fig 1}
\end{figure}
...
\textit{Node Pools: }The off-chain infrastructure and computational power of Inference Network is based around node pools, with epoch-based registration shown in Figure~\ref{fig:Fig 2}.
...
These pools consist of registered nodes that are assigned to process specific models' inference workloads. After registration, a model node becomes part of the available workforce. A node may service multiple models, and it is incumbent upon the node operator to ensure the successful completion of assigned tasks. Nodes pledge a certain amount of computational power, measured in FUCU (Foundational Universal Compute Units), to the network. With the computational requirements of workloads being predetermined, the network allocates tasks accordingly and does not exceed a node's capacity. A node is expected to fulfill its commitments within the epoch it is registered. Any failure to perform, or indication of unavailability, may result in slashing of staked collateral.

\textit{Aggregator Service: }The Aggregator is an off-chain service that coordinates the network. It submits inference tasks, probabilistically challenges completed tasks for verification, verifies EZKL proofs submitted by operators, and calls resolveTask to trigger rewards or slashing based on verification results. The aggregator role is designed to be permissionless, allowing any party to participate in network coordination.

\subsection{Persistent Storage}

Due to the considerable size of input and outputs from AI models, external persistent storage is required. Depending on the ultimate end use case of the output, storage within Inference Network may not be required. An example is an NFT image generated with a diffusion model. The hash of the image can be verified and stored on-chain with the image being stored on Arweave~\cite{Arweave} or other decentralized storage networks.
...
The on-chain component of the Inference Network system acts as the interface for end users and dApps.  Users or dApps submit workloads, which include all necessary details like input, precommitment, and destination. The on-chain architecture consists of smart contracts deployed on Ethereum: InferenceServiceManager (stake-backed security integration), InferenceTaskManager (task lifecycle orchestration), InferenceNodesManager (operator node registry and FUCU tracking), and ModelRegistry (model metadata and verification strategies). The security integration architecture is illustrated in Figure~\ref{fig:security-arch}.

\begin{figure}[htbp]
    \centering
    \scalebox{0.65}{
\tikz{
\tikzstyle{box}=[draw, inner sep=6pt, outer sep=0pt, minimum width=3cm, minimum height=0.8cm, align=center, font=\small]
\tikzstyle{container}=[draw, inner sep=10pt, outer sep=0pt, align=center]

\node[container, minimum width=10cm, minimum height=2.8cm] (security) {};
\node[above] at (security.north) {\textbf{Stake-Backed Security Layer}};
\node[box] (delegation) at (-2.5,0.3) {DelegationManager};
\node[box] (allocation) at (2.5,0.3) {AllocationManager};
\node[box] (rewards) at (0,-0.7) {RewardsCoordinator};

\node[box, below=1.5cm of security] (service) {InferenceServiceManager};

\node[box, below left=1cm and 0.5cm of service] (task) {TaskManager};
\node[box, below=1cm of service] (nodes) {NodesManager};
\node[box, below right=1cm and 0.5cm of service] (registry) {ModelRegistry};

\draw[-triangle 45] (security)--(service);
\draw[-triangle 45] (service)--(task);
\draw[-triangle 45] (service)--(nodes);
\draw[-triangle 45] (service)--(registry);
}
}
    \caption{Security Integration Architecture}
    \label{fig:security-arch}
\end{figure}

\subsection{Inference Market}

The ecosystem is anchored by an Inference Market. The protocol has a native queue of AI/ML workloads. A workload can be thought of as an end-to-end AI/ML Inference.
Each workload specifies all required details to complete it, such as input data, specific AI model for execution, output data requirements or on-chain execution.
Workloads posted to the network are priced according to their computational complexity.
Dynamic pricing may be used to adjust fees in real-time based on network congestion and demand urgency, incentivizing workers to prioritize high-value tasks with faster turnaround times by offering premium rates for rapid completion.
Simultaneously, it encourages users to offload less urgent or computationally lighter workloads during peak periods, balancing supply and demand while reducing queue delays.
By aligning monetary incentives with speed and congestion signals, the system optimizes resource allocation, ensuring efficient execution without overburdening the network.

To enhance efficiency in workload allocation, the Inference Market may employ a reverse Dutch auction framework.
Users submit jobs with a specified minimum acceptable price (reflecting their budget or urgency) and a maximum price they are willing to pay.
The protocol then dynamically escalates the job’s price over time, from the user’s minimum bid upward toward their maximum, until a worker accepts the task at the current price point.
This mechanism incentivizes workers to act quickly to secure lower prices while giving users control over their budget and desired urgency.
If no worker accepts the job by the max price threshold (or a predefined timeout), the task is either rejected or re-entered into the queue with updated pricing signals, allowing it to compete again under new conditions.

\subsection{Model Registry}

After circuitizing a model with the Inference Network SDK, its creator registers it on the network. This defines the required input and output data format, computational cost in FUCU, and the verification strategy (on-chain via verifier contract or off-chain via aggregator). The registry also stores the verifier contract address for on-chain verification and enforces registration fees to prevent spam.

\subsection{Model Node Pool Registration}

In order to complete inference workloads for a model, a model node must register its intent to complete workloads for specific models, as shown in Figure~\ref{fig:Fig 3}. Once registered the node is added to the pool of available nodes. Nodes may be registered to service multiple models, but it is the responsibility of the operator to ensure the node completes assigned workloads.
...
It is the expectation that a model node will complete delegated workloads during a registered epoch. Model nodes which fail to complete work while registered or otherwise shown to be unavailable will face slashing of their staked collateral.

Tasks follow a lifecycle managed by the InferenceTaskManager, as illustrated in Figure~\ref{fig:task-lifecycle}.

\begin{figure}[htbp]
    \centering
    \scalebox{0.65}{
\tikz{
\tikzstyle{state}=[draw, inner sep=0pt, outer sep=0pt, minimum width=2.4cm, minimum height=0.9cm, rounded corners=2mm, align=center, font=\small]
\tikzstyle{decision}=[draw, diamond, inner sep=0pt, outer sep=0pt, minimum width=1.6cm, minimum height=1.6cm, align=center, font=\scriptsize]

\node[state] (created) {CREATED};
\node[state, below=0.8cm of created] (assigned) {ASSIGNED};
\node[state, below=0.8cm of assigned] (completed) {COMPLETED};
\node[decision, below=0.8cm of completed] (challenge) {Challenge?};
\node[state, below=0.8cm of challenge] (challenged) {CHALLENGED};
\node[state, below=0.8cm of challenged] (resolved) {RESOLVED};
\node[state, right=1.5cm of resolved] (rejected) {REJECTED};

\draw[-triangle 45] (created)--node[right, font=\scriptsize] {sendTask} (assigned);
\draw[-triangle 45] (assigned)--node[right, font=\scriptsize] {submitOutput} (completed);
\draw[-triangle 45] (completed)--(challenge);
\draw[-triangle 45] (challenge)--node[right, font=\scriptsize] {Yes} (challenged);
\draw[-triangle 45] (challenge.west)--node[above, font=\scriptsize] {No} ++(-1.5,0)|-(resolved.west);
\draw[-triangle 45] (challenged)--node[right, font=\scriptsize] {Valid} (resolved);
\draw[-triangle 45] (challenged.east)--node[above, font=\scriptsize] {Invalid} ++(1.5,0)--(rejected.north);
}
}
...
As Inference Network will be an open permission-less network, no party (or even Inference Labs) can decide which models should or shouldn’t be available on the network. Instead an economic system determines how “good” a model is. This is crucial to retain an open and fair censorship free network.
...
\hline
\multicolumn{4}{|c|}{\textbf{Benchmarks using 8 Cores, 32 GB RAM, CUDA Accelerator}}  \\
\hline
...
\begin{enumerate}
    ...
    \item	Point ezkl to the .onnx and input of the model (as a .json file) to generate a ZK-SNARK circuit, represented by the following figure. From here a nearly 1:1 representation of the model is outlaid in a circuit.
...
This section first discusses remaining concerns such as IP risk and data privacy, then it explores future enhancements for Inference Network. We anticipate Inference Network's potential to support emerging technologies like FHEML, verifiable FHE, MPCML, and others. While these technologies currently pose computational challenges and are not yet practical, advancements in technology suggest that efficient solutions will become available over time. Consequently, Inference Network remains adaptable for updates to accommodate new technologies and services.

\subsection{Security evaluation}

The following provides an overview of the cybersecurity concerns present in the current version of Inference Network. However, these concerns are not significant enough to render Inference Network useless. The aim is to offer a comprehensive understanding of the advantages and disadvantages of Inference Network, providing potential customers with a clearer understanding.

\subsubsection{Reverse Engineering Risk}

One of the most valuable aspects of Inference Network is the aggregation of inferences. Having a clear picture of how often and by whom models are being utilized is a whole industry on its own; however this may create a new form of IP risk yet to be seen at scale. With a sufficient set of inputs to outputs from a particular model, a sophisticated 3rd party could train a similar or competing model using published data. Similar approaches have been seen by crowdsourcing prompt-to-response datasets from ChatGPT and then fine tuning GPTv2 to achieve surprisingly good results.
...
In the future, as AI models increasingly handle decision-making and various tasks, responsible AI will become even more critical than it is today. Simultaneously, with a shift towards decentralization, most communications and transactions are expected to occur on Web3. In such an environment, Inference Network will play a vital role by enabling AI model operators to broadcast proofs of honesty on Web3 without compromising the confidentiality of their model details.

\section{Risk Disclosures}

Prospective network participants should carefully consider the following risks.

\subsection{Technology Risk}
The Inference Network relies on complex smart contracts and cryptographic systems. Risks include:
\begin{itemize}
\item Smart contract vulnerabilities or bugs that could result in loss of funds or incorrect operation
\item Ethereum network failures, congestion, or protocol changes affecting service availability
\item Zero-knowledge proof system limitations or vulnerabilities
\item Dependencies on external infrastructure (storage networks, oracles)
\end{itemize}

\subsection{Operator Risk}
Network operators may:
\begin{itemize}
\item Fail to complete assigned inference tasks within required timeframes
\item Submit incorrect results, triggering slashing mechanisms
\item Experience hardware failures or connectivity issues affecting availability
\item Collude to manipulate network operations (mitigated by stake-backed security)
\end{itemize}

\subsection{Model Risk}
AI models deployed on the network may:
\begin{itemize}
\item Produce incorrect or biased outputs despite correct execution
\item Become outdated as training data ages
\item Contain vulnerabilities to adversarial inputs
\item Have verification costs that exceed practical limits for complex architectures
\end{itemize}

\subsection{Regulatory Risk}
Cryptocurrency and AI regulations vary by jurisdiction and are evolving. Future regulatory changes could restrict network operations, impose compliance requirements, or affect the legal classification of network services in certain jurisdictions.

\section{Environmental Impact}

\subsection{Consensus Mechanism}
Inference Network operates on Ethereum, which transitioned to Proof-of-Stake (PoS) in September 2022. The network no longer relies on energy-intensive Proof-of-Work mining.

\subsection{Energy Consumption}
\textbf{Ethereum Proof-of-Stake:} The Ethereum network's transition to PoS reduced its energy consumption by approximately 99.95\% compared to the previous Proof-of-Work mechanism. Inference Network inherits this efficiency.

\textbf{Inference Computation:} AI inference computations performed by network operators consume energy. However, this computation would occur regardless of blockchain verification. The zero-knowledge proof generation adds computational overhead but does not require specialized mining hardware.

\subsection{Environmental Considerations}
\begin{itemize}
\item No additional consensus mechanism beyond Ethereum PoS
\item No mining hardware requirements
\item Operators use standard computing infrastructure (GPUs, CPUs)
\item Proof generation is computationally intensive but temporary per task
\end{itemize}

The protocol does not introduce novel consensus mechanisms with significant environmental impact beyond the underlying Ethereum network.

\section{Conclusion}
Inference Network presents a comprehensive solution to the challenges of integrating AI and blockchain technology. It provides a decentralized protocol that enables secure, off-chain AI model inferences while preserving intellectual property through zero-knowledge cryptography. This innovative approach not only enhances privacy and security but also ensures the integrity and authenticity of AI models. The Inference Network architecture is designed to be modular and adaptable, supporting rapid deployment across multiple blockchain ecosystems. This work reﬂects a signiﬁcant step towards realizing a decentralized, secure, and privacy-preserving foundation for AI-enhanced blockchain systems, potentially revolutionizing the way AI operates in the blockchain space and contributing to the broader adoption of web3 technologies.
...
Inference Network aims to enable the decentralization and democratization of AI, aligning with core Web3 values. By facilitating privacy-preserving and veriﬁable AI services on public blockchains, Inference Network makes advanced AI accessible beyond large tech ﬁrms with proprietary data silos. This expands opportunities for innovation, collaboration, and value creation with AI systems operated transparently on open networks.

\textbf{* Developer experience-centric modular system design}

With a focus on simplicity and modular architecture, Inference Network streamlines the integration of cryptographically veriﬁed AI into decentralized applications. The system design centers on enhancing the developer experience through abstraction of complex zero-knowledge cryptography and seamless blockchain interoperability (zk-ML). Cost-reduction and ﬂexibility are built into the core framework to accommodate rapid evolution in the AI and blockchain landscape.

\textbf{* Open-source protocol for secure and composable systems}

As an open-source protocol, Inference Network fosters transparency, collective ownership, and community-driven development. Following the ethos of permissionless innovation, Inference Network creates infrastructure for AI-enhanced dApps to compose securely with minimal trust. By combining veriﬁed AI and blockchain building blocks within an open ecosystem, Inference Network aspires to be a public good facilitating the creation of services with embedded privacy, security and autonomy.

In summary, Inference Network implements the responsible and ethical application of AI within Web3 by making artiﬁcial intelligence both decentralized while protecting value creation. Through it’s innovative technical architecture and commitment to openness, Inference Network seeks to lay the foundations for the next generation of AI-powered decentralized applications.
